...
datadir = "NewOutputs/MiniPET/"

# New Outputs
## MiniPET
file_name = "Output_VerticalMotion_HighIntSource_800sec_coinc"
...
for i in range(df.ChannelIDL.count()):
    Geo_L.append(toNewGeoChannelID(df["ChannelIDL"][i]-896)) # Port 8 in FEB/D, "Left" module when facing in beam direction (downstream) -> PCB #43
    Geo_R.append(toNewGeoChannelID(df["ChannelIDR"][i])) # Port 1 in FEB/D, "Right" module when facing in beam direction (downstream) -> PCB #4
