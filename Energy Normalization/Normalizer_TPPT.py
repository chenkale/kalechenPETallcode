...
#file_name = "Test_Data"
dir = "NewOutputs/TPPT/" # Data file directory, if different from working directory
...
# TODO: Make sure this is correct (ChannelPairHeader_TPPT)
if (toReco):
    data[:,0] = data[:,0] 
    data[:,1] = data[:,1] 
    ...
    data[:,0] = data[:,0]
    data[:,1] = data[:,1]
    data[:,0] = np.vectorize(toGeoChannelID)(data[:,0])
    data[:,1] = np.vectorize(toGeoChannelID)(data[:,1])
print(data)
