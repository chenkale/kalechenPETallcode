...
'''
data_name = '80short.dat'
cut_on_photopeak = True # If false, row_length = 6
...
photopeak_dir = os.path.join(current_dir, '80short_ppbounds.txt')


if make_histogram:
    lor_pops = [0 for lr in range(128 * 128)]
if cut_on_photopeak:
    photopeakL = np.genfromtxt(photopeak_dir, dtype = np.float32)[128:] * 100
    photopeakR = np.genfromtxt(photopeak_dir, dtype = np.float32)[:128] * 100
...
for chunk in tqdm(pd.read_csv(calibration_data_file, sep = '\t', chunksize = 10000000, usecols = [2, 3, 4, 7, 8, 9], header = None)):
    ...
    chunk['IDL'] = chunk['IDL'] % 128           # Convert IDs to 0-3071 range for ease of handling, convert back when writing tsv
    chunk['IDR'] = chunk['IDR'] % 128                      
    ...
    if make_histogram:
        for row in chunk:
            lor_pops[int(row[1] * 128 + row[3])] += 1
    chunk_count += 1
