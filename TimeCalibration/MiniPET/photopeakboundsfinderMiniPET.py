...
name = '80short'
data_dir = f'/home/kale-chen/Documents/PET/TimeCalibration/MiniPET/{name}.bin'
out_dir = f'/home/kale-chen/Documents/PET/TimeCalibration/MiniPET/{name}_ppbounds.txt'

width = 2.5
#array_size = 3072 # TPPT
array_size = 128 # MiniPET


'''Begin script'''
num_rows = os.path.getsize(data_dir) // 12
data = np.memmap(data_dir, dtype = np.int16, mode = 'r', shape=(num_rows, 6))
print(num_rows)

# Event counts
eventcounts = np.zeros((array_size * array_size), dtype = np.int32)
for i in tqdm(range(0, num_rows, 1000000), desc='Event counts'):
    chunk = data[i:i+1000000]
    ids = np.asarray(chunk[:, [1, 3]], dtype = np.int32)
    indices = ids[:, 0] * array_size + ids[:, 1]
    np.add.at(eventcounts, indices, 1)
eventcounts = eventcounts.reshape((array_size, array_size))
...
photopeakboundsL = np.zeros((array_size, 2), dtype = np.float32)
photopeakboundsR = np.zeros((array_size, 2), dtype = np.float32)

chunksize = 64

for chunknum in tqdm(range(array_size // chunksize), desc = 'ID Chunk'):
    ...
    for idl in chunkidls:
        ...
        if fit[0] != 0:
            photopeakboundsL[idl][0] = fit[1] - width * fit[2]
            photopeakboundsL[idl][1] = fit[1] + width * fit[2]
        if idl % 100 == 0:
...
for chunknum in tqdm(range(array_size // chunksize), desc = 'ID Chunk'):
    ...
    for idr in chunkidrs:
        ...
        if fit[0] != 0: 
            photopeakboundsR[idr][0] = fit[1] - width * fit[2]
            photopeakboundsR[idr][1] = fit[1] + width * fit[2]
        if idr % 100 == 0:
