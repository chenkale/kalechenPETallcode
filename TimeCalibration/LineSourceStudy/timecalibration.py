...
calibration_data_dir = '/home/kale-chen/Documents/PET/TimeCalibration/LineSourceStudy/MovingSourceRun_vertical_OptimizedV2_OV_T1_2600_TrigOn_coinc_pcut.bin'

# File containing geometric offsets table, can generate from map with geotabler.py
geo_offsets_dir = '/home/kale-chen/Documents/PET/TimeCalibration/CalibrationDataPreparer/geometrictimeoffsets2.bin'

# Directory to output calibration tsvs to
output_dir = '/home/kale-chen/Documents/PET/TimeCalibration/LineSourceStudy'
...
numiterations = 10 # Number of calibration iterations
...
if start_from_dir == '':
    lookupl = np.zeros(3072, dtype = np.float32)
    lookupr = np.zeros(3072, dtype = np.float32)
else:
    lookup = np.genfromtxt(start_from_dir, delimiter = '\t')[:,4]
    lookupl = np.asarray(lookup[3072:6144], dtype = np.float32)
    lookupr = np.asarray(lookup[0:3072], dtype = np.float32)
...
for iteration in range(numiterations):
    ...
    # Loop
    for chunknum in tqdm(range(3072 // chunksize), desc = f'IDL'):
        ...
        for idl in tqdm(chunkidls, desc = 'Geo offsets', leave = False):
            ...
            #timediffs = timediffs + geooffsets[int(idl), idrs]
            timediffs = timediffs - geooffsets[int(idl), idrs] - lookupl[int(idl)] + lookupr[idrs]
    ...
    for chunknum in tqdm(range(3072 // chunksize), desc = f'IDR Chunk'):
        ...
        for idr in tqdm(chunkidrs, desc = 'Geo offsets', leave = False):
            ...
            #timediffs = timediffs + geooffsets[idls, int(idr)]
            timediffs = timediffs - geooffsets[idls, int(idr)] - lookupl[idls] + lookupr[int(idr)]
    ...
    # Write to tsv file
    tsv_dir = os.path.join(output_dir, 'tsvs')
