# Python script for generating time_offset_calibration.tsv for MiniPET 

'''
ONLY NEED TO CHANGE: center_source_data_dir, miniPET_geo_map_dir, output_dir
...
miniPET_geo_map_dir = 'C:/Users/burri/Downloads/testMiniPETmap.csv'
...
for r in range(128):
    output = pd.concat([output, pd.DataFrame([[0, 0, r // 64, r % 64, lookup_r[r]]])], ignore_index=True)
for l in range(128):
    output = pd.concat([output, pd.DataFrame([[0, 0, l // 64 + 14, l % 64, lookup_l[l]]])], ignore_index=True)
...
if generate_time_difference_histograms:
    ...
    plt.plot(np.linspace(-1*histogramlim, histogramlim, 300), triplegaussian(np.linspace(-1*histogramlim, histogramlim, 300), *popt0), color = 'red', label = 'before')
    print(popt0)
    ...
    popt1 = curve_fit(triplegaussian, bincenters1, hist_after[0], p0 = [6000, -5000, 1600, 30000, 0, 1000, 6000, 5000, 1600, 2500])[0]
    plt.plot(np.linspace(-1*histogramlim, histogramlim, 300), triplegaussian(np.linspace(-1*histogramlim, histogramlim, 300), *popt1), color = 'blue', label = 'after')
    print(popt1)
