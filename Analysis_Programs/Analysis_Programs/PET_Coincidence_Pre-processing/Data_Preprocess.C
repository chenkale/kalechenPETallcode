...
// HOW TO USE: To quickly and simply compile into a basic executable run the
// following command in terminal:
// %   g++ Data_Preprocess.C -o Data_Preprocess
...
int main(){
  ...
  bool Debug = 0;
  int lines_skipped = 0, lines_total = 0;
  std::string filename;
  ...
  while(in >> Size1 >> Index1 >> Time1 >> Energy1 >> AbsID1 >> Size2 >> Index2 >> Time2 >> Energy2 >> AbsID2){
    ...
    // Cut or skip lines with either energy below the threshold defined above
  ...
  printf("\n\n A total of %f percent of lines were dropped due to low energies \n\n",double(lines_skipped)/double(lines_total)*100);
