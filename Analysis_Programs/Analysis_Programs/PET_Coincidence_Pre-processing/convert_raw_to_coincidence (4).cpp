...
public:
	...
	void addEvents(float step1, float step2,EventBuffer<Coincidence> *buffer) {
		...
		for (int i = 0; i < N; i++) {
			...
			if(!e.valid) continue;
			if(e.nPhotons != 4) continue;

			GammaPhoton &p1 = *e.photons[0];
			GammaPhoton &p2 = *e.photons[1];
			GammaPhoton &p3 = *e.photons[2];
			GammaPhoton &p4 = *e.photons[3];

			int limit1 = (hitLimitToWrite < p1.nHits) ? hitLimitToWrite : p1.nHits;
			int limit2 = (hitLimitToWrite < p2.nHits) ? hitLimitToWrite : p2.nHits;
			int limit3 = (hitLimitToWrite < p3.nHits) ? hitLimitToWrite : p3.nHits;
			int limit4 = (hitLimitToWrite < p4.nHits) ? hitLimitToWrite : p4.nHits;

			for(int m = 0; m < limit1; m++) for(int n = 0; n < limit2; n++) for(int p = 0; p < limit3; p++) for(int j = 0; j < limit4; j++){
				if(m != 0 && n != 0 && p != 0 && j != 0) continue;
				Hit &h1 = *p1.hits[m];
				Hit &h2 = *p2.hits[n];
				Hit &h3 = *p3.hits[p];
				Hit &h4 = *p4.hits[j];

				int chip1 = (h1.raw->channelID % 4096) / 128;
				int chip2 = (h2.raw->channelID % 4096) / 128;
				int chip3 = (h3.raw->channelID % 4096) / 128;
				int chip4 = (h4.raw->channelID % 4096) / 128;

				float Eunit1 = h1.raw->qdcMode ? 1.0 : Tns;
				float Eunit2 = h2.raw->qdcMode ? 1.0 : Tns;
				float Eunit3 = h3.raw->qdcMode ? 1.0 : Tns;
				float Eunit4 = h4.raw->qdcMode ? 1.0 : Tns;

				//if (chip1 == chip2 || chip2 == chip3 || chip3 == chip1) continue;
				...
				if (fileType == FILE_ROOT){
					...
					fprintf(dataFile, "%d\t%d\t%lld\t%f\t%d\t%d\t%d\t%lld\t%f\t%d\t%d\t%d\t%lld\t%f\t%d\t%d\t%d\t%lld\t%f\t%d\n",
						p1.nHits, m,
						((long long)(h1.time * Tps)) + tMin,
						h1.energy * Eunit1,
						h1.raw->channelID,
						...
						p3.nHits, p,
            ((long long)(h3.time * Tps)) + tMin,
            h3.energy * Eunit3,
            h3.raw->channelID,

						p4.nHits, j,
            ((long long)(h4.time * Tps)) + tMin,
            h4.energy * Eunit4,
            h4.raw->channelID
