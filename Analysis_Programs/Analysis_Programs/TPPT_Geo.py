...
for i in range(128):
    geo_channels = pd.concat([geo_channels, {"AbsChannelID": i, "GeoChannelID": toGeo(i)}],ignore_index=True)
    ...
    for j in tqdm(range(12)):#12
        for k in range(8):#8
            for m in range(32):#32
                if i == 0:
                    df = pd.concat([df, {"RecoChannelID":toRecoChannelID(toAbsChannelID(GeoChannelID(i,j,k,m))),"x":nth_column_x(j,k), "y":nth_column_y(j,k), "z":nth_row_z(m)}],ignore_index=True)
                else:
                    df = pd.concat([df, {"RecoChannelID":toRecoChannelID(toAbsChannelID(GeoChannelID(i,j,k,m))),"x":-1*nth_column_x(j,k), "y":nth_column_y(j,k), "z":nth_row_z(m)}],ignore_index=True)

df = df.sort_values(by="RecoChannelID").reset_index(drop=True)
df.to_csv("C:/Users/burri/Downloads/testTPPTmap.csv", header=False, index=False)
