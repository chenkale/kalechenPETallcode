...
class Channel_Pair:
    ...
    def CTRFit(self):
        ...
        low = TRes5_UN.min()
        high = TRes5_UN.max()
        #changed to always keep the bin size constatn to keep comparison possible
        num_bins = 600
    ...
    def PlotEnergySpectrum(self, display=False):
        ...
        t_x = 1
        plt.text(t_x,t_y[0], 'Channel ID {}'.format(toGeoChannelID(LChannel)))
        ...
        plt.text(t_x,t_y[5],'Channel ID {}'.format(toGeoChannelID(RChannel)))
...
#PART1
t = time.time()
chunks = pd.read_csv(dir+"CW2PRW1POW10_coinc.dat", sep="\t", header=None, names=["TimeL", "ChargeL", "ChannelIDL","TimeR", "ChargeR", "ChannelIDR"], usecols=[2,3,4,7,8,9],low_memory=False,chunksize=100000)
...
DATA.to_csv(dir+"CW2PRW1POW10_DATA.csv".format(run_num))
...
#     Geo_L.append(toGeoChannelID(df["ChannelIDL"][i]))
#     Geo_R.append(toGeoChannelID(df["ChannelIDR"][i]))
...
# # Hist1D(data.CTR, units = "ns", fit = gaussFit(data.CTR))
...
# #
# # #Best ERes for a PCB
# # EResB_data, EResBcolumn = getChipData(data,0)
...
# # plt.show()
#
# PP_L = data.groupby("ChannelIDL").PP_Count.sum().reset_index()
# PP_L.columns = ["ChannelIDL", "Counts"]
# PP_R = data.groupby("ChannelIDR").PP_Count.sum().reset_index()
# PP_R.columns = ["ChannelIDR", "Counts"]
# print(PP_L)
#
# Hist1D(PP_L.Counts, units="Channel ID")
# plt.show()
