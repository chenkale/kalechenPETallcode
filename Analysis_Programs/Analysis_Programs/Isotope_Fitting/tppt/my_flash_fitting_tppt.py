...
def plot_histogram_with_fit(df, fitter, fit_function, args, geo_channels, bin_centers, values, y_shift):
    """
    ...
    ax.bar(bin_centers, values + y_shift, width=[5 for _ in range(len(bin_centers))],
    ...
    fig.suptitle(f"Reduced $\chi^{{2}}$: {reduced_chi_squared:.2f}", fontsize=30)
        ...
        for b in bins_lengths:
            ...
            for index in range(len(pos_bins)):
                ...
                fig, ax, ax_residuals,  bin_centers = plot_histogram_with_fit(
                    group, fitter, fit_function, args, geo_channels, bin_centers, values,y_shift
                )
                ...
                if len(dfs) > 1:
                    ...
                    export_fit_results_latex_table(fitter,  args, f'/home/michaelgajda/thesis_work/single_segment_fits_mini/{args.data_file[:-4]}', 0,dof)
